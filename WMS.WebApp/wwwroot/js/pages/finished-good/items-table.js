(function () {
                ...
                render: function (data, type, row) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        const editButton = canEdit
                            ? `<a href="/FinishedGood/EditItem?hu=${hu}&receiveId=${receiveId}&grouped=${showGrouped}" 
            ...
            { data: 'location', title: 'Location' },
            {
                ...
                render: function (hu, type, row) {
                    if (row.hasEditAccess) {
                        return `<a href="/FinishedGood/EditItem?hu=${hu}&receiveId=${receiveId}&grouped=${showGrouped}" 
                            class="w-8 h-8 bg-warning-100 dark:bg-warning-600/25 text-warning-600 dark:text-warning-400 rounded-full inline-flex items-center justify-center"
                            title="Edit Item">
                            <iconify-icon icon="lucide:edit"></iconify-icon>
                        </a>`;
    ...
    function initItemsTable() {
        ...
        itemsTable = $('#items-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: `/FinishedGood/GetPaginatedItems?receiveId=${receiveId}&grouped=${showGrouped}`,
                type: 'POST',
