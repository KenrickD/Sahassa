    ...
    document.querySelectorAll('.release-type').forEach(select => {
        select.addEventListener('change', function () {
            ...
            if (this.value === 'Full') {
                ...
                // Update the toggle all items button text
                const toggleAllItemsButton = row.querySelector('.toggle-all-items');
                if (toggleAllItemsButton) {
                    toggleAllItemsButton.textContent = 'Hide All Items';
                }
            } else {
    ...
    document.querySelectorAll('.receive-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            ...
            if (this.checked) {
                ...
                const toggleAllItemsButton = row.querySelector('.toggle-all-items');
                if (toggleAllItemsButton) {
                    toggleAllItemsButton.textContent = 'Show All Items';
                }
            }
        });
...
function collectReleaseData() {
    ...
    const explicitlyReleasedPallets = new Set();
    ...
    document.querySelectorAll('.pallet-checkbox:checked').forEach(checkbox => {
        if (checkbox.dataset.released !== 'true') {
            ...
            const receiveId = checkbox.dataset.receiveId;
            explicitlyReleasedPallets.add(palletId);
    ...
    document.querySelectorAll('.item-checkbox:checked').forEach(checkbox => {
        if (checkbox.dataset.released !== 'true') {
            ...
            if (explicitlyReleasedPallets.has(palletId)) {
                return;
            }
...
// Function to submit the release
function submitRelease() {
    // Get the data
    const releaseData = collectReleaseData();

    // Get all selected item IDs that aren't part of selected pallets
    const selectedItems = [];
    document.querySelectorAll('.item-checkbox:checked').forEach(checkbox => {
        if (checkbox.dataset.released !== 'true') {
            const itemId = checkbox.dataset.itemId;
            ...
            if (!releaseData.palletReleases.some(pr => pr.palletId === palletId)) {
    ...
    if (selectedItems.length === 0 && releaseData.palletReleases.length === 0) {
    ...
    // Create the payload
    const payload = {
        finishedGoodId: window.finishedGoodId,
        itemIds: selectedItems,
        itemReleaseDates: releaseData.itemReleaseDates,
        palletReleases: releaseData.palletReleases
    };
    ...
    fetch('/FinishedGood/Submit', {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Redirect
                window.location.href = '/FinishedGood/Datatable';
            } else {
                // Show error message
                const errorContainer = document.getElementById('errorContainer');
                if (errorContainer) {
