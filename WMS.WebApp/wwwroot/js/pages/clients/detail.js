(function () {
    ...
    function setupClientForm() {
        $("#clientForm").on("submit", function (e) {
            ...
            const clientId = $('#currentClientId').val();
            var isEdit = false;
            ...
            const warehouseSelect = $(this).find('select[name="Client.WarehouseId"]');
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        if (data.errors) {
                            ...
                            showErrorToast(data.message || "An error occurred while saving the client.");
                        }
                    }
                },
                error: function (xhr) {
                    console.error('Client save error:', xhr);
                    ...
                    if (xhr.responseJSON) {
                        ...
                        if (errors) {
                            ...
                            showErrorToast(xhr.responseJSON.message || "An error occurred while saving the client.");
                        }
                    } else {
                        showErrorToast("An error occurred while saving the client.");
                    }
                },
                complete: function () {
                    // Reset button state
    ...
    function initializeConfigurationManagement() {
        ...
        $('#configForm input').on('change', function () {
    ...
    function loadClientConfiguration() {
        ...
        const clientId = $('#currentClientId').val();
        if (!clientId || clientId === '00000000-0000-0000-0000-000000000000') {
            console.log('No client ID found for configuration');
            return;
        }
    ...
    function getCurrentConfigFormData() {
        return {
            requiresQualityCheck: $('#requiresQualityCheck').is(':checked'),
            autoGenerateReceivingLabels: $('#autoGenerateReceiving').is(':checked'),
            autoGenerateShippingLabels: $('#autoGenerateShipping').is(':checked'),
            // Fix these lines - use the correct input names instead of IDs
            handlingFeePercentage: parseFloat($('input[name="HandlingFeePercentage"]').val()) || 0,
            storageFeePerCubicMeter: parseFloat($('input[name="StorageFeePerCubicMeter"]').val()) || 0,
            defaultLeadTimeDays: parseInt($('input[name="DefaultLeadTimeDays"]').val()) || 0,
            lowStockThreshold: parseInt($('input[name="LowStockThreshold"]').val()) || 0,
            sendLowStockAlerts: $('#sendLowStockAlerts').is(':checked'),
            allowPartialShipments: $('#allowPartialShipments').is(':checked'),
            requiresAppointmentForReceiving: $('#requiresAppointment').is(':checked')
        };
    }
    ...
    function saveConfigurationChanges() {
        const clientId = $('#currentClientId').val();
        if (!clientId || clientId === '00000000-0000-0000-0000-000000000000') {
            showErrorToast('Client ID not found');
            return;
        }
        ...
        // Prepare data for submission - include all client data
        const formData = new FormData();

        // Get all client form data
    ...
    function resetConfigurationChanges() {
        ...
        $('#requiresQualityCheck').prop('checked', originalConfigData.requiresQualityCheck);
        $('#autoGenerateReceiving').prop('checked', originalConfigData.autoGenerateReceivingLabels);
        $('#autoGenerateShipping').prop('checked', originalConfigData.autoGenerateShippingLabels);
        // Fix these lines too
        $('input[name="Client.HandlingFeePercentage"]').val(originalConfigData.handlingFeePercentage);
        $('input[name="Client.StorageFeePerCubicMeter"]').val(originalConfigData.storageFeePerCubicMeter);
        $('input[name="Client.DefaultLeadTimeDays"]').val(originalConfigData.defaultLeadTimeDays);
        $('input[name="Client.LowStockThreshold"]').val(originalConfigData.lowStockThreshold);
        $('#sendLowStockAlerts').prop('checked', originalConfigData.sendLowStockAlerts);
        $('#allowPartialShipments').prop('checked', originalConfigData.allowPartialShipments);
        $('#requiresAppointment').prop('checked', originalConfigData.requiresAppointmentForReceiving);
